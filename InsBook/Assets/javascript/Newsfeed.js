                ...
                success: function (response) {
                    ...
                    if (response.status == true) {
                        ...
                        response.baiviet.forEach(function (post) {
                            html = html + '<article class="post-box post-' + post.id + '">' +
                                '                                <header class="post-title">' +
                                '                                    <div class="post-info">' +
                                '                                        <img src="' + baseUrl + '/Images/' + post.avatarnguoidang + '" class="rounded rounded-circle lazy" width="50px" height="50px" alt="">' +
                            ...
                            if (post.ganthe.length != 0) {
                                ...
                                post.ganthe.forEach(function (ganthe) {
                                    html = html + '<a href="/Client/Personal/Index/' + ganthe["id"] + '"><b>' + ganthe["ten"] + ', </b></a>';
                                });
                                html = html + '</span>';
                            }
                            ...
                            html = html + '                                                    </ul>' +
                                ...
                                '                                                            <label for="tools-img-' + post.id + '" class="edit-tools-item" id="edit-post-anh-default-' + post.id + '"><img src="' + baseUrl + '/Images/default-adding-post.png" alt="" /></label>' +
                                '                                                        </div>' +
                                '                                                    </div>' +
                                ...
                                '                                                        </div>' +
                                '                                                        <input type="text" class="form-control" id="edit-post-chude-' + post.id + '" placeholder="Chủ đề bài viết ?">' +
                                '                                                    </div>' +
                            ...
                            if (post.anh.length != 0) {
                                ...
                                if (post.anh.length == 1) {
                                    ...
                                    html = html + '<p class="text-center">Chức năng đang cập nhật</p>';
                                }
                                html = html + ' </div>';
                            }
                            html = html + ' </div>' +
                            ...
                            if (post.dsnguoithich.some(x => x.idnguoithich == response.userID)) {
                                html = html + '                                            <button type="button" class="btn btn-light post-like-icon liked" id="post-like-icon-' + post.id + '">' +
                                    '                                                <i class="fas fa-heart"></i>' +
                                    '                                            </button>';
                            }
                            else {
                                html = html + '                                            <button type="button" class="btn btn-light post-like-icon" id="post-like-icon-' + post.id + '">' +
                                    '                                                <i class="fas fa-heart"></i>' +
                                    '                                            </button>';
                            }
                            html = html + '                                        <button type="button" class="btn btn-light" id="comment-icon-' + post.id + '"><i class="far fa-comment"></i></button>' +
                            ...
                            if (post.comment.length > 0) {
                                html = html + '<div id="comment-list-' + post.id + '" class="comment-list">';
                                post.comment.forEach(function (item) {
                                    html = html + '                                                    <div class="comment-item" id="comment-item-' + item.comment_id + '">' +
                                    ...
                                    if (item.comment_child.length > 0) {
                                        html = html + '<div class="comment-item-childs" id="comment-item-childs-' + item.comment_id + '">' +
                                        ...
                                        item.comment_child.forEach(function (item_child) {
                                            html = html + '<div class="comment-item-child">' +
                                                '                                                                            <div class="user-ava">' +
                                    ...
                                    html = html + '<section class="comment-bar-child" id="comment-bar-child-' + item.comment_id + '">' +
                                        '                                                            <div class="user-ava">' +
                                        '                                                                <img src="' + baseUrl + '' + baseUrl + '/Images/22904751_688560231353134_2748711313877205190_o.jpg" alt="Alternate Text" />' +
                                        '                                                            </div>' +
                                        '                                                            <div class="form-group">' +
                            ...
                            html = html + '</div>' +
                                ...
                                '                                        <button type="button" class="btn btn-light post-comment-button" id="post-comment-button-' + post.id + '">Đăng</button>' +
                                    ...
                                    '                                </div>';
                            if (post.anh.length > 0) {
                                post.anh.forEach(function (modal_anh) {
                                    html = html + '                                        <!-- Modal -->' +
                                        ...
                                        '                                                        <div class="post-content">' +
                                        '                                                            <img class="" src="' + baseUrl + '/Images/' + modal_anh.anh_url + '" alt="">' +
                                        '                                                        </div>' +
                                        '                                                    </div>' +
                                        ...
                                        '                                                            </div>' +
                                        '                                                            <section class="post-icons">';
                                    if (post.dsnguoithich.some(x => x.idnguoithich == response.userID) && post.id == modal_anh.id) {
                                        html = html + '<button type="button" class="btn btn-light post-like-icon liked" id="modal-post-like-icon-' + modal_anh.id + '">' +
                                            '                                                                        <i class="fas fa-heart"></i>' +
                                            '                                                                    </button>';
                                    }
                                    else {
                                        html = html + '<button type="button" class="btn btn-light post-like-icon" id="modal-post-like-icon-' + modal_anh.id + '">' +
                                            '                                                                        <i class="fas fa-heart"></i>' +
                                            '                                                                    </button>';
                                    }
                                    html = html + '<button type="button" class="btn btn-light"><i class="far fa-comment"></i></button>' +
                            ...
                            html = html + '<div class="modal fade modal-share" id="modal-share-' + post.id + '" tabindex="-1" role="dialog">' +
                                '                                    <div class="modal-dialog modal-dialog-centered" role="document">' +
                                '                                        <div class="modal-content">' +
                                '                                            <div class="modal-header">' +
                                ...
                                '                                            </div>' +
                                '                                            <div class="modal-footer">' +
                                '                                                <button type="button" class="btn btn-primary btn-block" onclick="SharePost(' + post.id + ', ' + response.userID + ')">Đăng</button>' +
                                '                                            </div>' +
                                '                                        </div>' +
                                '                                    </div>' +
                                '                                </div>' +
